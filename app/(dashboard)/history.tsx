...
import { 
  CheckCircle2, Calendar, TrendingUp, Sparkles, Camera, 
...
// ==========================================
// 🚀 MOCK DATA (Khớp với hình ảnh của bạn)
// ==========================================
const MOCK_HISTORY = [
  {
    id: 'batch-17787633',
...
export default function HistoryScreen() {
  ...
  // Thay thế bằng useSystem() thật của bạn sau này
  const batchHistory = MOCK_HISTORY;

  // Thống kê tĩnh theo hình ảnh để hiển thị đẹp nhất
  ...
  // Helper cho Badge Rủi ro
  const getRiskBadge = (risk: string) => {
    switch (risk) {
      case 'high':
        return <View className="bg-rose-500/20 px-2 py-0.5 rounded border border-rose-500/30"><Text className="text-rose-400 text-[10px] font-bold">Rủi ro cao</Text></View>;
      case 'medium':
        return <View className="bg-amber-500/20 px-2 py-0.5 rounded border border-amber-500/30"><Text className="text-amber-400 text-[10px] font-bold">Rủi ro TB</Text></View>;
      case 'low':
        return <View className="bg-emerald-500/20 px-2 py-0.5 rounded border border-emerald-500/30"><Text className="text-emerald-400 text-[10px] font-bold">Rủi ro thấp</Text></View>;
      default: return null;
    }
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#0f172a]" edges={['top']}>
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
        ...
        {/* ================================================= */}
        {/* 1. TOP 4 STATS CARDS (Phối màu giống hệt bản Web) */}
        {/* ================================================= */}
            ...
            <View className="flex-1 flex-row justify-between items-end px-2 z-10">
              {[4.2, 5.2, 4.0, 4.3, 4.6, 4.8, 4.3, 4.6].map((val, idx) => (
                <View key={idx} className="items-center">
                  {/* Cột */}
                  <View 
                    className="w-7 bg-blue-500 rounded-t-md shadow-[0_0_10px_rgba(59,130,246,0.3)]" 
        ...
        <View className="flex-row justify-between items-center mb-4">
          ...
          <View className="bg-slate-800 px-3 py-1 rounded-full border border-slate-700">
            <Text className="text-slate-300 text-xs">{totalBatches} batch</Text>
          </View>
        </View>
